...
function RaiseComplaint({ user, setPage }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            alert("Complaint submitted successfully and is awaiting Technician acceptance!");
            setPage("dashboard");
        } catch (error) {
            console.error("Submission failed", error);
            const errorMessage = error.response?.data?.message || "Failed to submit. Try again.";
        ...
        <div className="min-h-screen bg-dark-bg text-gray-100 font-sans selection:bg-brand selection:text-black">
            ...
            <div className="relative z-10 max-w-2xl mx-auto px-6 py-16">
                <div className="mb-10 text-center">
                    <h2 className="text-4xl font-bold tracking-tight text-white mb-2">Raise a Complaint</h2>
                    <p className="text-gray-400">Provide details and snap a photo of the issue so our AI can help classify it.</p>
                </div>
                ...
                <div className="p-8 sm:p-10 bg-dark-card border border-white/5 rounded-[2rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)]">
                    <form onSubmit={handleSubmit} className="space-y-8">
                        ...
                        <div className="space-y-3">
                            <label className="text-sm font-semibold text-gray-300 ml-1">Upload Issue Image</label>
                            <div className="w-full flex-col relative border-2 border-dashed border-white/10 rounded-2xl p-10 flex items-center justify-center hover:border-brand/40 bg-dark-bg/30 transition-all group">
                                ...
                                <div className="w-16 h-16 rounded-full bg-white/5 flex items-center justify-center mb-4 group-hover:scale-110 group-hover:bg-brand/10 transition-all">
                                    <svg className="w-8 h-8 text-gray-400 group-hover:text-brand transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9zM15 13a3 3 0 11-6 0 3 3 0 016 0z" />
                                    </svg>
                                </div>
                                <div className="text-center">
                                    <p className="text-brand font-semibold mb-1">Click to upload or drag & drop</p>
                        ...
                        {/* Description field */}
                        <div className="space-y-3">
                            <label className="text-sm font-semibold text-gray-300 ml-1">Issue Description (Optional)</label>
                            <textarea 
